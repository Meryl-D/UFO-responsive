...
function renderMarkers(map, shapeData) {
  ...
  var randSightings = (0, _ufoData.getRandSightings)(shapeData); // Ajoute les marqueurs

  randSightings.forEach(function (sighting) {
    var marker = _leaflet["default"].marker([parseFloat(sighting.latitude), parseFloat(sighting.longitude)], {
      icon: alienPin
    }).addTo(map);
