...
exports.ufoData = exports.parseYear = void 0;

var _complete = _interopRequireDefault(require("/data/ufo-sighting/complete.csv"));
...
var cleanedData = _complete["default"].filter(function (d) {
  return d.shape != null && d.comments != null && d.latitude != null && d.longitude != null && d.country === 'us' && parseTime(d.datetime) >= parseTime('01/01/1940 00:00');
...
var ufoData = cleanedData.filter(function (d) {
  return d.shape === 'light' || d.shape === 'circle' || d.shape === 'triangle' || d.shape === 'unknown' || d.shape === 'fireball';
});
exports.ufoData = ufoData;

function filterShapeData() {
...
function getRandSightings() {
  var shapeData = filterShapeData();
  var randSightings = [];
...
function getDateYearData() {
  return getStringYearData().map(function (d) {
    return parseYear(d);
  });
...
function countEntries() {
  var stringYearData = getStringYearData();
  var count = {};
...
function getMaxAmountRounded() {
  var maxRounded = Math.ceil(d3.max(Object.values(countEntries())) / 100) * 100;
